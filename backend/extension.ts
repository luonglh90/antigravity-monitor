...
import { fetchCredentialsFromLanguageServer } from './localLanguageServerClient';
...
async function attemptAutoLoginWithRetry(
  ...
  for (let attempt = 0; attempt < maxRetries; attempt++) {
    try {
      console.log(`[Extension] Auto-login attempt ${attempt + 1}/${maxRetries}`);

      // Fetch credentials from language server
      const credentials = await fetchCredentialsFromLanguageServer();

      if (!credentials) {
        console.log('[Extension] No credentials available from language server');
      ...
      try {
        ...
        if (!existingAccount) {
          await accountManager.addAccount(credentials.email, credentials.token);
          console.log(`[Extension] New account added: ${credentials.email}`);
        } else {
          console.log(`[Extension] Account already exists: ${credentials.email}`);
        }
      } catch (error: any) {
      ...
      console.error(`[Extension] Auto-login attempt ${attempt + 1} failed:`, error);
...
export function activate(context: vscode.ExtensionContext) {
  ...
  let disposable = vscode.commands.registerCommand('antigravity-monitor.open', async () => {
    ...
    if (!fs.existsSync(htmlPath)) {
      vscode.window.showErrorMessage('Antigravity Monitor UI not found. Please build the extension first.');
      panel.webview.html = `
        <!DOCTYPE html>
        <html lang="en">
        <body>
    ...
    html = html.replace(/(href|src)="([^"]*)"/g, (match, p1, p2) => {
      ...
      if (p2.startsWith('http') || p2.startsWith('data:') || p2.startsWith('vscode-webview:')) {
        return match;
      }
    ...
    const initializeBackendState = async () => {
      ...
      try {
        ...
        console.log('[Extension] Found', existingAccounts.length, 'existing accounts');
        
        if (existingAccounts.length > 0) {
          // If we have existing accounts, try to set the first one as active
          const firstAccount = existingAccounts[0];
          console.log('[Extension] Setting existing account as active:', firstAccount.email);
          await accountManager.setActiveAccount(firstAccount.email);
          
          // Send success message immediately
          panel.webview.postMessage({ command: 'autoLoginSuccess', account: firstAccount });
          
          // Fetch quota for the account
          try {
            console.log('[Extension] Fetching quota for existing account...');
            const quota = await quotaService.fetchQuotaForAccount(firstAccount.email, firstAccount.token);
            console.log('[Extension] Quota fetched successfully');
            panel.webview.postMessage({ command: 'quotaData', data: quota });
          } catch (error: any) {
            console.error('[Extension] Failed to fetch quota for existing account:', error);
            panel.webview.postMessage({ command: 'quotaError', error: 'Failed to fetch quota data' });
          }
          
          console.log('[Extension] Account manager initialized successfully with existing account');
          return; // Skip auto-login since we have an existing account
        }
        
        console.log('[Extension] No existing accounts found, proceeding with auto-login');
      } catch (error) {
        console.error('[Extension] Failed to initialize account manager:', error);
        panel.webview.postMessage({ command: 'error', error: 'Failed to load accounts. Please try again.' });
      }
      ...
      // Attempt auto-login
      const autoLoginSuccess = await attemptAutoLoginWithRetry(accountManager);
      console.log('[Extension] Auto-login result:', autoLoginSuccess);

      if (autoLoginSuccess) {
        // Requirement 1.3: Navigate to Dashboard on success
        const activeAccount = accountManager.getActiveAccount();
        console.log('[Extension] Active account after auto-login:', activeAccount?.email);
        
        if (activeAccount) {
          panel.webview.postMessage({ command: 'autoLoginSuccess', account: activeAccount });

          // Fetch quota for active account
          try {
            console.log('[Extension] Fetching quota for active account...');
            const quota = await quotaService.fetchQuotaForAccount(activeAccount.email, activeAccount.token);
            console.log('[Extension] Quota fetched successfully');
            panel.webview.postMessage({ command: 'quotaData', data: quota });
          } catch (error: any) {
            console.error('[Extension] Failed to fetch quota after auto-login:', error);
            panel.webview.postMessage({ command: 'quotaError', error: 'Failed to fetch quota data' });
          }
        }
      } else {
      ...
      async (message) => {
        try {
          if (message.command === 'webviewReady') {
            console.log('[Extension] Webview is ready, initializing backend state');
            ...
            try {
              ...
              let session = await vscode.authentication.getSession('google', ['email'], { createIfNone: true });
              ...
              if (session && session.account && session.account.label) {
                ...
                // Fetch quota
                const quota = await quotaService.fetchQuotaForAccount(email, token);
                panel.webview.postMessage({ command: 'loginSuccess', account: { email, token }, quota });
              } else {
                throw new Error('Failed to get authentication session');
              }
            } catch (error: any) {
              console.error('[Extension] Login failed:', error);
              panel.webview.postMessage({ command: 'error', error: `Login failed: ${error.message}` });
            }
            ...
            try {
              ...
              if (session && session.account && session.account.label) {
                ...
                const existingAccount = accountManager.getAccount(email);
                if (existingAccount) {
                  panel.webview.postMessage({ command: 'error', error: `Account ${email} already exists` });
                  return;
                }

                // Add account
                await accountManager.addAccount(email, token);
                panel.webview.postMessage({ command: 'accountAdded', account: { email, token } });
              } else {
                throw new Error('Failed to get authentication session');
              }
            } catch (error: any) {
              console.error('[Extension] Add account failed:', error);
              panel.webview.postMessage({ command: 'error', error: `Failed to add account: ${error.message}` });
            }
            ...
            try {
              ...
              // Requirement 12.2: Clear session data when switching accounts
              ...
              const quota = await quotaService.fetchQuotaForAccount(email, account.token);
              panel.webview.postMessage({ command: 'accountSwitched', account, quota });
            } catch (error: any) {
              console.error('[Extension] Switch account failed:', error);
              panel.webview.postMessage({ command: 'error', error: `Failed to switch account: ${error.message}` });
            }
            ...
            try {
              ...
              // Invalidate cached quota for removed account
              ...
              if (newActiveAccount) {
                const quota = await quotaService.fetchQuotaForAccount(newActiveAccount.email, newActiveAccount.token);
                panel.webview.postMessage({ command: 'accountRemoved', account: newActiveAccount, quota });
              } else {
                panel.webview.postMessage({ command: 'accountRemoved', account: null });
              }
            } catch (error: any) {
              console.error('[Extension] Remove account failed:', error);
              panel.webview.postMessage({ command: 'error', error: `Failed to remove account: ${error.message}` });
            }
            ...
            try {
              const activeAccount = accountManager.getActiveAccount();
              if (!activeAccount) {
                panel.webview.postMessage({ command: 'quotaError', error: 'No active account' });
                return;
              }

              // Requirement 15.2: Display cached data immediately while fetching fresh data
              const cachedQuota = quotaService.getCachedQuota(activeAccount.email);
              if (cachedQuota) {
                panel.webview.postMessage({ command: 'quotaData', data: cachedQuota, cached: true });
              }

              // Fetch fresh quota in background
              quotaService.backgroundRefreshQuota(activeAccount.email, activeAccount.token).catch(error => {
                console.error('[Extension] Background quota refresh failed:', error);
              });

              // Also fetch fresh quota immediately
              const quota = await quotaService.fetchQuotaForAccount(activeAccount.email, activeAccount.token);
              panel.webview.postMessage({ command: 'quotaData', data: quota, cached: false });
            } catch (error: any) {
              console.error('[Extension] Fetch quotas failed:', error);
              panel.webview.postMessage({ command: 'quotaError', error: error.message ?? String(error) });
            }
          }
        } catch (error: any) {
          console.error('[Extension] Unexpected error handling message:', error);
          panel.webview.postMessage({ command: 'error', error: 'An unexpected error occurred' });
        }
      },
      undefined,
